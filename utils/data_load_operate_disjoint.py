...
def sampling_other_data(gt_re, class_count, ratio_train, ratio_val, ratio_test):
    ...
    for cls in range(class_count):
        cls_index_all = np.where(gt_re == cls + 1)[0]
        cls_index_train = cls_index_all[:int(ratio_train*len(cls_index_all))]
        cls_index_test = cls_index_all[int(ratio_train*len(cls_index_all)):]
        np.random.shuffle(cls_index_test)
        # print(f"cls_index_train_len = {len(cls_index_train)}, cls_index_test_len = {len(cls_index_test)}")
...
def generate_iter_hu_w_val(data_padded, data, gt_train_re, gt_test_re, index, patch_length, batch_size, model_type_flag,
    ...
    if model_type_flag == 1:  # data for single spatial net
        ...
        spe_train_samples = HSI_create_pathes(data_padded_torch, data_torch, index[0], patch_length, 2) # vector
...
def generate_iter_other(data_padded, data, gt_re, index, patch_length, batch_size, model_type_flag,
                           model_3D_spa_flag):
    ...
    # for data label
    train_labels = gt_re[index[0]] - 1
    val_labels = gt_re[index[1]] - 1
    test_labels = gt_re[index[2]] - 1
    ...
    # for data
    if model_type_flag == 1:  # data for single spatial net
        spa_train_samples = HSI_create_pathes(data_padded_torch, data_torch, index[0], patch_length, 1)# b h w c
        spa_val_samples = HSI_create_pathes(data_padded_torch, data_torch, index[1], patch_length, 1)
        spa_test_samples = HSI_create_pathes(data_padded_torch, data_torch, index[2], patch_length, 1)

        spa_train_samples = spa_train_samples.permute(0,3,1,2)
        spa_val_samples = spa_val_samples.permute(0,3,1,2)
        spa_test_samples = spa_test_samples.permute(0,3,1,2)
        ...
        spa_train_samples = HSI_create_pathes(data_padded_torch, data_torch, index[0], patch_length, 1).permute(0,3,1,2)
        spa_val_samples = HSI_create_pathes(data_padded_torch, data_torch, index[1], patch_length, 1).permute(0,3,1,2)
        spa_test_samples = HSI_create_pathes(data_padded_torch, data_torch, index[2], patch_length, 1).permute(0,3,1,2)

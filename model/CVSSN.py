...
class CVSSN(nn.Module):
    def __init__(self, in_channels, h, w, class_count):
        ...
        if self.in_channels % self.win_spa_size != 0:
            pad_len = self.win_spa_size - self.in_channels % self.win_spa_size # 71
        else:
            pad_len = 0
    ...
    def forward(self, x_spa, x_spe):# torch.Size([32, 11, 11, 50]) torch.Size([32, 50])

        x_spa = x_spa.permute(0, 3, 1, 2)
        batch_size, c, h, w = x_spa.size()

        # SSIF
        x_spe = torch.unsqueeze(torch.unsqueeze(x_spe, 1), 1) # [B, 1, 1, in_channels]
        # 使用 F.pad 替代 ReflectionPad2d，因为当 pad_len 超过输入维度时，ReflectionPad2d 会失败
        # padding 格式: (pad_left, pad_right, pad_top, pad_bottom) 对应 (width_left, width_right, height_top, height_bottom)
        # 使用 'replicate' 模式复制边界值，比 'reflect' 更稳定
        if self.pad_len > 0:
            x_spe_paded = F.pad(x_spe, pad=(0, self.pad_len, 0, 0), mode='replicate')
        else:
            x_spe_paded = x_spe
...
if __name__ == "__main__":
    ...
    # 模型参数配置
    in_channels = 200  # 光谱波段数
    patch_size = 9  # 空间补丁大小 (h = w = 9)
    num_classes = 16  # 分类类别数
    batch_size = 4  # 批次大小
    
    # 创建模型
    model = CVSSN_(in_channels=in_channels, h=patch_size, w=patch_size, num_classes=num_classes).to(device)
    model.eval()
    
    # 创建测试输入
    # x_spa: 空间数据，形状为 (batch, h, w, in_channels)
    x_spa = torch.randn(batch_size, patch_size, patch_size, in_channels).to(device)
    # x_spe: 光谱数据，形状为 (batch, in_channels)
    x_spe = torch.randn(batch_size, in_channels).to(device)
    
    print(f"\n输入形状:")
    print(f"  x_spa (空间数据): {x_spa.shape}")
    print(f"  x_spe (光谱数据): {x_spe.shape}")
    
    # 前向传播测试
    print("\n开始前向传播...")
    try:
        with torch.no_grad():
            output = model(x_spa, x_spe)
        ...
        print("\n测试反向传播...")
        model.train()
        x_spa.requires_grad = True
        x_spe.requires_grad = True
        output = model(x_spa, x_spe)
        loss = output.sum()
        loss.backward()
        print("✓ 反向传播成功！")
        ...
        with torch.no_grad():
            # 测试不同的补丁大小
            test_patch_sizes = [7, 11, 13]
            for test_patch in test_patch_sizes:
                test_x_spa = torch.randn(batch_size, test_patch, test_patch, in_channels).to(device)
                test_x_spe = torch.randn(batch_size, in_channels).to(device)
                test_model = CVSSN_(in_channels=in_channels, h=test_patch, w=test_patch, num_classes=num_classes).to(device)
                test_output = test_model(test_x_spa, test_x_spe)
                assert test_output.shape == (batch_size, num_classes), \
                    f"补丁大小 {test_patch} 的输出形状错误: {test_output.shape}"
                print(f"  ✓ 补丁大小 {test_patch}x{test_patch} 测试通过")
